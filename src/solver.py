...
from src.loss import loss_maxcut_numpy_boost, loss_sat_numpy_boost, loss_maxind_numpy_boost, loss_maxind_QUBO, loss_task_numpy, loss_task_numpy_vec, loss_mincut_numpy_boost, loss_watermark, loss_partition_numpy, loss_partition_numpy_boost, loss_QGT_numpy
...
def BER(true,predict):
    ...
    error=false_negative+false_negative
...
def centralized_solver(constraints, header, params, file_name):
    ...
    for constraint in constraints:
        if params['data'] == 'task' or params['data']=='QGT':
    ...
    if params['data']!='task' and params['data']!='QGT' : ################################################### MAHDI
    ...
    if params['random_init']=='none' and not load:
        ...
        if params['data'] != 'hypergraph' and params['data'] != 'task' and params['data'] != 'uf' and params['data'] != 'NDC' and params['data'] != 'QGT':
    ...
    for i in range(params['K']): ## if we want to solve the problem for K number of times ##
        ...
        elif params['mode'] == 'QGT':
            score = loss_QGT_numpy(res,constraints, [1 for i in range(len(constraints))], params['lasso_coef'])
            # score_old, score1, _ = loss_maxind_numpy_boost(res, constraints, [1 for i in range(len(constraints))],
            #                                                inc=params['boosting_mapping'])
            score_th = loss_QGT_numpy(res_th,constraints , [1 for i in range(len(constraints))], params['lasso_coef'])
        ...
        sols.append(res)
        sols_th.append(res_th)
        

    return reses, reses_th, sols,sols_th, probs, timeit.default_timer() - temp_time, train_times, map_times
