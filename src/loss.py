...
# def loss_QGT_weighted(res, C, weights):  ###################################### MAHDI
#     loss = 0
#     x = res.squeeze()
#     #weight2=torch.ones((len(res),))
#     #weight2[0:int(np.floor(len(res)/2))]=2*weight2[0:int(np.floor(len(res)/2))]
#     for c, w in zip(C, weights):
#         temp=torch.sum([x[index-1] for index in c[:-1]])
#         temp1 = w *(c[-1]-temp)**2
#         loss = loss+temp1
#         # print("loss",loss)
#     # temp2 = 0.2*sum([min(1 - x[index], x[index]) for index in range(len(x))])
...
def loss_QGT_weighted(res, C, weights, lasso_coef):
    loss = 0
    x = res.squeeze()
    ...
    for c, w in zip(C, weights):
        temp=0
        # Convert c[:-1] to a tensor and adjust for 1-based indexing
        # indices = torch.tensor(c[:-1]) - 1  # Assuming c is a list of indices
        for index in c:
            temp+= (x[index-1])
        # temp = torch.sum(x[indices])  # Use advanced indexing to directly sum the elements
        ####Quad loss
        temp1 = w * (c[-1] - temp) ** 2  # Calculate the loss component
        ####l1 loss
        #temp1 =torch.abs( w * (c[-1] - temp) )
        loss += temp1  # Accumulate the loss

    #reg=torch.sum(res)
    #L1
    #reg=torch.abs(torch.sum(res)-20)

    #quad
    #reg=(torch.sum(res)-20)**2
    #loss+=lasso_coef*reg
    
    return loss


def loss_QGT_numpy(res, C, weights, lasso_coef):  ###################################### MAHDI
    loss = 0
    #weight2=torch.ones((len(res),))
    #weight2[0:int(np.floor(len(res)/2))]=2*weight2[0:int(np.floor(len(res)/2))]
    ...
    #L1
    #reg=np.abs(np.sum(list(res.values()))-20)
    #quadratic
    #reg=(np.sum(list(res.values()))-20)**2
    

    #reg=np.sum(list(res.values()))
    #loss+=lasso_coef*reg
    return loss
