...
import torch
import timeit
from src.loss import loss_sat_numpy, loss_maxcut_numpy, loss_maxcut_numpy, loss_maxind_numpy, loss_maxind_QUBO, loss_task_numpy, loss_task_numpy_vec, loss_mincut_numpy, loss_partition_numpy, loss_QGT_numpy
import random
...
def mapping_distribution(best_outs, params, n, info, weights, constraints, all_weights, inc, penalty, hyper):
    ...
    if params['mode'] == 'sat':
        ...
        _loss = loss_QGT_numpy

    for rea in range(params['N_realize']):
        res = {x: np.random.choice(range(2), p=[1 - best_outs[x], best_outs[x]]) for x in best_outs.keys()}
        best_score = _loss(res, constraints, all_weights, params['lasso_coef'])
        ...
        for it in range(params['Niter_h']):
            ...
            for i in ord:
                ...
                lt = _loss(temp, info[i], weights[i], params['lasso_coef'])
                l1 = _loss(res, info[i], weights[i], params['lasso_coef'])
            ...
            #     best_res = copy.deepcopy(res)
            #     best_score = score
        score = _loss(res, constraints, all_weights, params['lasso_coef'])
        #print(score)
        if score < best_score:
            best_res =copy.deepcopy(res)
            best_score = score
