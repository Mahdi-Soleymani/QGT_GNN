...
def centralized_train(G, params, f, C, n, info, file_name):
    ...
    for i in range(rounds):
        ...
        elif params['mode'] == 'QGT':  ####################################### MAHDI 
            loss = loss_QGT_weighted(temp, C, [1 for i in range(len(C))], params['lasso_coef'])
            #print('Loss:', loss)
            if loss==0:
                print("found zero loss")
                break

        # for name, param in [(n, p) for n, p in conv1.named_parameters() if p.requires_grad]:
        ...
        #     print(f"Conv1 weight gradient: {conv1.weight.grad}")
        # else:
        #     print("No gradient for conv1 weight")

        # if conv2.weight.grad is not None:
        #     print(f"Conv2 weight gradient: {conv2.weight.grad}")
        # else:
        #     print("No gradient for conv2 weight")

        # # Similarly check gradients for biases if they exist
        # if conv1.bias is not None and conv1.bias.grad is not None:
        #     print(f"Conv1 bias gradient: {conv1.bias.grad}")
        # if conv2.bias is not None and conv2.bias.grad is not None:
        #     print(f"Conv2 bias gradient: {conv2.bias.grad}")
    ...
    if params['data'] != 'task' and params['data'] !='QGT':
